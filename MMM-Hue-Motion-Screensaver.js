...
Module.register("MMM-Hue-Motion-Screensaver", {
  ...
  start: function () {
    ...
    if (!this.config.hueHost || !this.config.sensorId || !this.config.apiKey) {
      this.logError(
        "Missing required config: hueHost, sensorId, and apiKey must all be set.",
      )
      return
    }

    this.sendSocketNotification("INIT_CONFIG", {
  ...
  checkMotion: function () {
    this.sendSocketNotification("CHECK_MOTION", null)
  },
    ...
    if (
      ...
      if (this.state === 1) {
        ...
        const timeRemaining = Math.max(
          0,
          Math.floor((this.nextScreenOffTime - now) / 1000),
        )
